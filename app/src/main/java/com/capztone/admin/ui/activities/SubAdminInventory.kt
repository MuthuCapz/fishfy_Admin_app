...
class SubAdminInventory: AppCompatActivity() {
    ...
    private val binding:  ActivitySubAdminInventoryBinding by lazy {
        ActivitySubAdminInventoryBinding.inflate(layoutInflater)
    }
...
// Start a delay to hide the loading indicator after 1500 milliseconds (1.5 seconds)
        ...
        if (noProductTextView != null && button != null) {
            adapter =  SubInventoryAdapter(this, listOf(), databaseReference, noProductTextView, button)
                            ...
                            for (ref in deleteOperations) {
                                ref.removeValue()
                                    .addOnSuccessListener {
                                        ...
                                        Toast.makeText(this@SubAdminInventory, "Item deleted successfully", Toast.LENGTH_SHORT).show()
                                        retrieveMenuItem()
                                    }
                                    .addOnFailureListener { e ->
                                        Log.e("DeleteItem", "Failed to delete item: $e")
                                        Toast.makeText(this@SubAdminInventory, "Failed to delete item", Toast.LENGTH_SHORT).show()
                                    }
                            }
                        }
