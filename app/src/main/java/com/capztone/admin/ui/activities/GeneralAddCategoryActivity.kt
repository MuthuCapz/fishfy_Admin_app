...
import com.capztone.admin.databinding.ActivityGeneralAddCategoryBinding
...
class GeneralAddCategoryActivity : AppCompatActivity() {

    private lateinit var binding: ActivityGeneralAddCategoryBinding
    private var imageUri: Uri? = null
    private var shopId: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityGeneralAddCategoryBinding.inflate(layoutInflater)
        setContentView(binding.root)

        // Fetch shop id from "Delivery Details -> Shop Id"
        fetchShopId()
...
// Check if the category name matches variations of "discount" or common misspellings
    ...
    private fun fetchShopId() {
        val shopIdRef = FirebaseDatabase.getInstance().reference
            .child("Delivery Details")
            .child("Shop Id") // Changed to ShopId path
        ...
        shopIdRef.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onCancelled(databaseError: DatabaseError) {
                Toast.makeText(this@GeneralAddCategoryActivity, "Failed to retrieve shop ID", Toast.LENGTH_SHORT).show()
            }
        })
    }
